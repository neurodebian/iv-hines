...
void MACcanvas::damage(Coord left, Coord bottom, Coord right, Coord top)
{
	...
	//printf("damage this  l:%d r:%d t:%d b:%d\n", area.left, area.right, area.top, area.bottom);
...
// -----------------------------------------------------------------------
// Changes the brush to use when rendering a path -- NOT YET IMPLEMENTED
// -----------------------------------------------------------------------
	...
	Coord y)                        // y-coordinate
{
	...
	if (text_item_.count == 0) {
      ...
      || (!Math::equal(float(to_pixels(tx, Dimension_X)), next_x, 1.1))) {//allow 1 pixel slop
        flush();
        begin_item(tx, ty, width);
    } 
