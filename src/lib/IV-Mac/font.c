	...
	int style_flags)       			// desired style attributes
{
	...
	//before other Interviews windows are created.
		...
		GetPort(&oldPort);
		SetPort(SIOUXTextWindow);
		short save_font = SIOUXTextWindow->txFont;
	    short save_face = SIOUXTextWindow->txFace;
	    short save_size = SIOUXTextWindow->txSize;
	    short save_mode = SIOUXTextWindow->txMode;
...
{
	printf("const char* Font::name not yet implemented\n");	//This still needs to be done
	return nil;
}
