...
void EventRep::setWindow(WindowPtr aMacWindow){
	CWindowPeek theWindowRecord;
	WindowRep*	temp;
	
	theWindowRecord = (CWindowPeek)aMacWindow;
	
	temp = (WindowRep*)theWindowRecord->refCon;
	if(temp){
...
void EventRep::mouseDownEventHook(void){
	...
	if(theEvent_->modifiers & controlKey){
		button_ = Event::right;
	} else if (theEvent_->modifiers & shiftKey){
	...
	switch (windowPart)
	{
		...
		case inDrag:
			...
			if ( (hitWindow != frontWindow) && !(theEvent_->modifiers & cmdKey))
			{
				...
				else 
				{
					...
					if (ourStructure = (WindowRep*)((CWindowPeek)hitWindow)->refCon)
					{
			...
			else if (ourStructure = (WindowRep*)((CWindowPeek)hitWindow)->refCon)	/* if it's our window */
			{
		...
		case inGrow:
			if (ourStructure = (WindowRep*)((CWindowPeek)hitWindow)->refCon)	/* if it's our window */
			{
		...
		case inGoAway:
			...
			if (TrackGoAway(hitWindow, theEvent_->where))
			{
				...
				if (theEvent_->modifiers & optionKey)	
				{
					...
					ourStructure = (WindowRep*)((CWindowPeek)hitWindow)->refCon;
		...
		case inContent:
			...
			if (hitWindow != frontWindow){
				...
				if ( windowType == MACwindow::MODAL || windowType == MACwindow::MOVABLE_MODAL){
					SysBeep(30);	/* modal window in front, sorry Charlie */
				} else if (ourStructure = (WindowRep*)((CWindowPeek)hitWindow)->refCon)	/* hit a background EasyWindow */
				{
			...
			else if ((ourStructure = (WindowRep*)((CWindowPeek)frontWindow)->refCon) &&
...
void EventRep::mouseUpEventHook(void){
	...
	if(MACwindow::isOurWindow(frontWindow)){
		ourStructure = (WindowRep*)((CWindowPeek)frontWindow)->refCon;
	} else {
		ourStructure = nil;
	}
...
void EventRep::osEventHook(void){
	...
	if(eventType & mouseMovedMessage){
		...
		if((WindowRep*)((CWindowPeek)aMacWindow)->refCon)
			((WindowRep*)((CWindowPeek)aMacWindow)->refCon)->MACinput(theEvent_, type_, Event::none);
