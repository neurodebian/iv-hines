...
void WindowRep::bind()
{
	...
	stretch_.h = Math::min((r.x_requirement().natural() + r.x_requirement().stretch()),2000.);
	stretch_.v = Math::min((r.y_requirement().natural() + r.y_requirement().stretch()), 2000.);
...
void WindowRep::doGrow(EventRecord* theEvent){
	...
	win->glyph_->undraw();
	((MACcanvas*)win->canvas_)->initClip();
