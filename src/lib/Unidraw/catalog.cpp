...
static const char* HexEncode (
    ...
    unsigned int r = iv26_round(ir * color_base);
    unsigned int g = iv26_round(ig * color_base);
    unsigned int b = iv26_round(ib * color_base);
...
static const char* HexGrayEncode (
    ...
    unsigned int gray = iv26_round(igray * color_base);
...
void Catalog::ReadExtraData (
    ...
    for (int i = 0; !in.eof() && !FoundDelim(delim, *extra_data); ++i) {
        char c;
        in.get(c);
        extra_data->Insert((void*) (long) c, i);
    }
}
...
PSColor* Catalog::ReadColor (istream& in) {
    ...
    if (buf[0] == 'c') {
        ...
        if (defined && in.good()) {
            int ir = iv26_round(r * float(0xffff));
            int ig = iv26_round(g * float(0xffff));
            int ib = iv26_round(b * float(0xffff));
...
static int CalcBitmap (float graylevel) {
    ...
    return shades[iv26_round(graylevel * (SHADES - 1))];
}
...
inline long ObjectMapElem::GetId () { return (long) _elem->tag(); }
